...
class StationModel(Document):
    ...
    loaded_samples = fields.IntField(
        default=0
    )  # cannot use min value, breaks dec__ operator
    ...
    assigned_station_op = fields.EmbeddedDocumentField(
        StationOpDescriptorModel, null=True
    )
    station_op_history = fields.EmbeddedDocumentListField(
        StationOpDescriptorModel, default=[]
    )
    requested_robot_op_history = fields.EmbeddedDocumentListField(
