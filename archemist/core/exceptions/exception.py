...
class StationNoOutcomeError(Error):
    def __init__(self, station_name):
        self.message = f"{station_name} station hasn not completed and \
            can not provide an outcome result"
...
class DatabaseNotPopulatedError(Error):
    def __init__(self):
        self.message = (
            "The provided database is not existing or does not have any documents!!!"
        )
