...
class ChemSpeedRackSm(StationProcessFSM):
    def __init__(self, station: Station, params_dict: Dict):
        ...
        states = [
            ...
            State(
                name="navigate_to_chemspeed", on_enter=["request_navigate_to_chemspeed"]
            ),
            State(
                name="retreat_from_chemspeed",
                on_enter=["request_navigate_to_chemspeed"],
            ),
            State(
        ...
        transitions = [
            ...
            },
            {
                ...
                "dest": "navigate_to_chemspeed",
                "conditions": "is_station_job_ready",
            },
            {
                "trigger": self._trigger_function,
                "source": "navigate_to_chemspeed",
                "dest": "open_chemspeed_door",
                "conditions": "is_station_job_ready",
            },
            {
                "trigger": self._trigger_function,
                "source": "open_chemspeed_door",
                "dest": "load_batch",
                "unless": "is_station_operation_complete",
                "conditions": "is_station_job_ready",
            },
            {
                "trigger": self._trigger_function,
            ...
            },
            {
                ...
                "dest": "load_batch",
                "unless": "are_all_batches_loaded",
                "conditions": "is_station_job_ready",
            },
            {
                "trigger": self._trigger_function,
                "source": "added_batch_update",
                "dest": "close_chemspeed_door",
                "conditions": ["is_station_job_ready", "are_all_batches_loaded"],
            },
            {
                "trigger": self._trigger_function,
                "source": "close_chemspeed_door",
            ...
            },
            {
                ...
                "dest": "retreat_from_chemspeed",
                "unless": "is_station_operation_complete",
                "conditions": "is_station_job_ready",
            },
            {
                "trigger": self._trigger_function,
                "source": "retreat_from_chemspeed",
                "dest": "chemspeed_process",
                "conditions": "is_station_job_ready",
            },
            {
                "trigger": self._trigger_function,
                "source": "chemspeed_process",
                "dest": "disable_auto_functions",
                "conditions": "is_station_job_ready",
                "before": "process_batches",
            },
            {
                "trigger": self._trigger_function,
                "source": "open_chemspeed_door",
                "dest": "unload_batch",
                "conditions": ["is_station_operation_complete", "is_station_job_ready"],
            },
            {
                "trigger": self._trigger_function,
                "source": "unload_batch",
    ...
    def request_load_batch(self):
        robot_job = KukaLBRTask.from_args(
            name="LoadChemSpeed",
            params=[True, self._status["batch_index"] + 1],
    ...
    def request_navigate_to_chemspeed(self):
        self._station.request_robot_op(
            KukaNAVTask.from_args(Location(26, 1, ""), False)
        )  # TODO get this property from the config
    ...
    def request_process_operation(self):
        ...
        if isinstance(current_op, CSCSVJobOpDescriptor):
            ...
            combined_op = CSCSVJobOpDescriptor.from_args(
                dispense_info=combined_dispense_info
            )
    ...
    def process_batches(self):
        ...
        for batch in self._station.assigned_batches:
            for i in range(0, batch.num_samples):
                ...
                sample_op.dispense_info = {
                    k: v[i]
                    for k, v in batch.recipe.get_current_task_op().dispense_info.items()
                }
