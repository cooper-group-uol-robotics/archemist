...
class QuantosSolidDispenserQS2ROSHandler(StationHandler):
    ...
    def execute_op(self):
        ...
        if isinstance(current_op, QuantosDispenseOpDescriptor):
            # Dispense solid
            self._quantos_pub.publish(
                quantos_command=QuantosCommand.DISPENSESOLID,
                quantos_int=self._station.carousel_pos,
                quantos_float=current_op.dispense_mass,
            )
            ...
            self._quantos_pub.publish(
                quantos_command=QuantosCommand.SETSAMPLEPOS,
                quantos_int=current_op.carousel_pos,
            )
        else:
