...
class ArchemistCLI:
    ...
    def _process_indivdual_station_menu(self, station_name: str, station_id: int):
        ...
        if selection["station_menu"] == "Repeate assigned op":
            print(
                f"Repeating assigned op for station {station_name} \
                    with id: {station_id}"
            )
            msg = CMDMessage(
                category=CMDCategory.STATION,
                cmd="repeat_op",
                params=[station_name, station_id],
            )
            self._client.send_json(msg.to_json())
