...
class InputStationSm(BaseSm):
    ...
    def __init__(self, station: Station, params_dict: Dict):
        ...
        self.machine.add_transition('process_state_transitions',source='init_state',dest='pickup_rack', prepare='update_assigned_batches', conditions='all_batches_assigned')
        self.machine.add_transition('process_state_transitions',source='pickup_rack',dest='removed_batch_update', conditions='is_station_job_ready')
        self.machine.add_transition('process_state_transitions',source='removed_batch_update',dest='pickup_rack', unless='are_all_batches_unloaded', conditions='is_station_job_ready')
