# External
import rospy
from kmriiwa_chemist_msgs.msg import (
    TaskStatus,
    LBRCommand,
    NavCommand,
    KMRStatus,
...
class KmriiwaROSHandler(RobotHandler):
    def __init__(self, robot: Robot):
        ...
        rospy.Subscriber(
            "/kuka2/kmr/task_status", TaskStatus, self._kmr_task_cb, queue_size=1
        )
        rospy.Subscriber(
            "/kuka2/lbr/task_status", TaskStatus, self._lbr_task_cb, queue_size=1
        )

        # TODO add callbacks to check on the robot status so that charging or
        # other operations can performed to run the process smoothly.
        rospy.Subscriber(
            "/kuka2/lbr/robot_status",
            LBRStatus,
            self._update_lbr_status_cb,
            queue_size=2,
        )
        rospy.Subscriber(
            "/kuka2/kmr/robot_status",
            KMRStatus,
            self._update_kmr_status_cb,
            queue_size=2,
        )
    ...
    def run(self):
        try:
            # update local counter since robot cmd counter is ahead while
            # we restarted (self._lbr_cmd_seq = 0)
    ...
    def _kmr_task_cb(self, msg):
        # TODO if published task sequence != 0, while local task counter == 0 it
        # means we restarted and thus set local task counter = published task counter
    ...
    def _lbr_task_cb(self, msg):
        if (
            msg.task_name != ""
            and msg.task_name == self._lbr_task_name
            and msg.cmd_seq == self._lbr_cmd_seq
        ):
            if msg.task_state == TaskStatus.FINISHED:
                self._lbr_task_name = ""
                self._lbr_done = True
        ...
        if self._kmr_task is not None:
            if self._kmr_cmd_seq != -1:
                if self._kmr_task.fine_localization:
                    self._kmr_task_name = f"fine_nav to n:{self._kmr_task.node_id} \
                                            g:{self._kmr_task.graph_id}"
                else:
                    self._kmr_task_name = (
                        f"nav to n:{self._kmr_task.node_id} g:{self._kmr_task.graph_id}"
                    )
                rospy.loginfo(
    ...
    def is_op_execution_complete(self):
        ...
        if self._kmr_task is not None and self._kmr_done:
            ...
            self._robot.location = Location(
                node_id=self._kmr_task.node_id,
                graph_id=self._kmr_task.graph_id,
                frame_name="",
            )
            self._kmr_task = None
            self._kmr_done = False
