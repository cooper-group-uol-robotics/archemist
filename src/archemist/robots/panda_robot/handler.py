...
class PandaROSHandler(RobotHandler):
    def __init__(self, robot: Robot):
        ...
        # TODO robot topic can be set from the config file or even be
        # associated with the robot id
        self._panda_pub = rospy.Publisher("/panda1/task", PandaTask, queue_size=1)
        rospy.Subscriber(
            "/panda1/task_status", TaskStatus, self._panda_task_cb, queue_size=2
        )
    ...
    def run(self):
        try:
            # update local counter since robot cmd counter is ahead while we
            # restarted (self._panda_cmd_seq = 0)
    ...
    def _panda_task_cb(self, msg: TaskStatus):
        if (
            ...
            and msg.task_name == self._panda_task.task_name
            and msg.task_seq == self._panda_task.task_seq
        ):
