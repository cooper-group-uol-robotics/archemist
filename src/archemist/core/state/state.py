...
class State:
    ...
    def stations(self) -> List[Station]:
        return [
            StationFactory.create_from_model(model) for model in StationModel.objects
        ]
    ...
    def batches_buffer(self) -> StateObjListAdapter:
        return StateObjListAdapter(
            model=self._model, field_name="batches_buffer", state_obj_cls=Batch
        )
    ...
    def robot_ops_queue(self):
        return OpListAdapter(
            model=self._model, field_name="robot_ops_queue", factory_cls=RobotFactory
        )
    ...
    def recipes_queue(self):
        return StateObjListAdapter(
            model=self._model, field_name="recipes_queue", state_obj_cls=Recipe
        )
    ...
    def get_robots(self, robot_type: str):
        return [
            RobotFactory.create_from_model(model)
            for model in RobotModel.objects(_type=robot_type)
        ]
