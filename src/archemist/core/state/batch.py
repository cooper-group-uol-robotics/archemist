...
class Sample:
    ...
    def operation_ops(self):
        return [
            ...
            for op_model in self._model.operation_ops
        ]
...
class Batch:
    ...
    def process_current_sample(self):
        ...
        if self.current_sample_index == self.num_samples:
            ...
            self._log_batch(
                "All samples have been processed. Batch index is reset to 0."
            )
    ...
    def add_station_op_to_current_sample(self, station_op: StationOpDescriptor):
        ...
        self._model.update(
            **{f"samples__{self._model.current_sample_index}": current_sample.model}
        )
        # https://stackoverflow.com/questions/2932648/how-do-i-use-a-string-as-a-keyword-argument
        # self._model.update(samples=self._model.samples) alternatively but the
        # whole list will be updated
