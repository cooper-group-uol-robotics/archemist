...
class Station:
    ...
    def station_op_history(self) -> List[StationOpDescriptor]:
        ...
        return [
            StationFactory.create_op_from_model(op_model)
            for op_model in self._model.station_op_history
        ]
    ...
    def requested_robot_op_history(self) -> List[Any]:
        ...
        return [
            RobotFactory.create_op_from_model(robot_job_model)
            for robot_job_model in self._model.requested_robot_op_history
        ]
    ...
    def assigned_batches(self) -> List[Batch]:
        ...
        if self._model.assigned_batches:
            assigned_batches_list = [
                Batch(batch_model) for batch_model in self._model.assigned_batches
            ]
    ...
    def processed_batches(self) -> List[Batch]:
        ...
        if self._model.processed_batches:
            processed_batches_list = [
                Batch(batch_model) for batch_model in self._model.processed_batches
            ]
    ...
    def process_assigned_batches(self):
        ...
        self._log_station("all assigned batches processing complete.")
    ...
    def complete_robot_op_request(self, complete_robot_op: RobotOpDescriptor):
        ...
        self._log_station("Robot job request is fulfilled.")
        self._update_state(StationState.PROCESSING)
    ...
    def complete_assigned_station_op(self, success: bool, **kwargs):
        self._model.reload("assigned_station_op")
        station_op = StationFactory.create_op_from_model(
            self._model.assigned_station_op
        )
        ...
        self._log_station("Station op is complete.")
