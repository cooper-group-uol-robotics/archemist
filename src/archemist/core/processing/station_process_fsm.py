...
class StationProcessFSM:
    ...
    def init_state_machine(self, states: List[State], transitions: Dict):
        ...
        self.machine = Machine(self, states=states, initial=self._status['state'], transitions=transitions)
    ...
    def update_batch_loc_to_station(self):
        self._station.assigned_batches[self._status['batch_index']].location = self._station.location
    ...
    def update_batch_loc_to_robot(self):
        ...
        self._station.assigned_batches[self._status['batch_index']].location = Location(-1,-1,f'{last_executed_robot_op.robot_stamp}/Deck')
