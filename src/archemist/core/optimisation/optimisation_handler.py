...
class OptimisationHandler:
    def __init__(self, optimiser: Type[OptimiserBase], optimisation_records: OptimisationRecords, state: State,
        ...
        if optimisation_records.save_opt_model_path:
            save_path = Path(optimisation_records.save_opt_model_path)
            ...
            for batch in completed_batches:
                if batch.id not in self._optimisation_records.batches_seen:
                    ...
                    if self._opt_model_save_path:
                        self._optimiser.update_model(result_data_pd, save_model_to = self._opt_model_save_path)
                    else:
                        self._optimiser.update_model(result_data_pd)
