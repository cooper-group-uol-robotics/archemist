...
class StationProcessModel(Document):
    ...
    requested_by = fields.ObjectIdField(
        null=True)  # station that requested the process
    # station that will be running the process
    assigned_to = fields.ObjectIdField(null=True)
    # station associated with the process
    associated_station = fields.StringField(required=True)
    status = fields.EnumField(
        enum=ProcessStatus, default=ProcessStatus.INACTIVE)
    ...
    ''' robot ops '''
    req_robot_ops = fields.ListField(
        fields.ReferenceField(RobotOpModel), default=[])
    robot_ops_history = fields.ListField(
        fields.ReferenceField(RobotOpModel), default=[])

    '''station ops'''
    operation_specs_map = fields.MapField(
        field=fields.EmbeddedDocumentField(OperationSpecsModel), default={})
    req_station_ops = fields.ListField(
        fields.ReferenceField(StationOpModel), default=[])
    station_ops_history = fields.ListField(
        fields.ReferenceField(StationOpModel), default=[])

    '''station processes'''
    req_station_procs = fields.ListField(
        fields.ReferenceField("self"), default=[])
    station_procs_history = fields.ListField(
        fields.ReferenceField("self"), default=[])
