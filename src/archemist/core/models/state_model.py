...
class WorkflowStateModel(Document):
    ...
    robot_ops_queue = fields.ListField(
        fields.ReferenceField(RobotOpModel), default=[])
    station_op_requests_queue = fields.ListField(
        fields.ReferenceField(StationOpModel), default=[])
    proc_requests_queue = fields.ListField(
...
class InputStateModel(Document):
    ...
    batches_queue = fields.ListField(
        fields.ReferenceField(BatchModel), default=[])
    recipes_queue = fields.ListField(
        fields.ReferenceField(RecipeModel), default=[])
