...
class StationOpModel(Document):
    ...
    associated_station = fields.StringField(
        required=True)  # station associated with the op

    outcome = fields.EnumField(OpOutcome, null=True)
    results = fields.ListField(fields.ReferenceField(
        StationOpResultModel), default=[])

    start_timestamp = fields.ComplexDateTimeField(null=True)
    end_timestamp = fields.ComplexDateTimeField(null=True)

    meta = {'collection': 'station_ops',
