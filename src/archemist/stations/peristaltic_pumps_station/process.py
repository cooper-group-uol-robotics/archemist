...
class PandaPumpSolubilityProcess(StationProcess):
    def __init__(self, process_model: Union[StationProcessModel, ModelProxy]) -> None:
        ...
        self.STATES = [State(name='init_state'),
                       ...
                       State(name='move_dispense_head',
                             on_enter='request_move_dispense_head'),
                       State(name='dispense_liquid',
                             on_enter='request_dispense_liquid'),
                       State(name='replace_dispense_head',
                             on_enter='request_replace_dispense_head'),
        ...
        self.TRANSITIONS = [
            ...
            {'source': 'move_dispense_head', 'dest': 'dispense_liquid',
                'conditions': 'are_req_robot_ops_completed'},
            {'source': 'dispense_liquid', 'dest': 'replace_dispense_head',
                'conditions': 'are_req_station_ops_completed'},
            {'source': 'replace_dispense_head', 'dest': 'final_state',
                'conditions': 'are_req_robot_ops_completed'}
        ]
    ...
    def request_move_dispense_head(self):
        robot_op = RobotTaskOp.from_args(
            name='move_dispense_head', target_robot="PandaRobot")
    ...
    def request_dispense_liquid(self):
        ...
        current_op = self.generate_operation(
            "dispense_op", target_sample=sample)
        self.request_station_op(current_op)

    def request_replace_dispense_head(self):
        robot_op = RobotTaskOp.from_args(
            name='replace_dispense_head', target_robot="PandaRobot")
        self.request_robot_ops([robot_op])
