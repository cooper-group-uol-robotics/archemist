...
class StationLoadingSm(StationProcessFSM):
    ...
    def request_load_batch(self):
        robot_job = (KukaLBRTask.from_args(name=self._batch_load_task,params=[False,self._status['batch_index']],
                                    type=RobotTaskType.UNLOAD_FROM_ROBOT,location=self._station.location))
        current_batch_id = self._station.assigned_batches[self._status['batch_index']].id
        self._station.request_robot_op(robot_job,current_batch_id)

    def request_unload_batch_job(self):
        robot_job = (KukaLBRTask.from_args(name=self._batch_unload_task,params=[False,self._status['batch_index']],
                                    type=RobotTaskType.UNLOAD_FROM_ROBOT,location=self._station.location))
        current_batch_id = self._station.assigned_batches[self._status['batch_index']].id
        self._station.request_robot_op(robot_job,current_batch_id)
    ...
    def process_batch(self):
        ...
        for _ in range(0, self._station.assigned_batches[self._status['batch_index']].num_samples):
