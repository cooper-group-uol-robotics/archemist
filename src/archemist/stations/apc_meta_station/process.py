...
class APCSynthesisProcess(StationProcess):
    def __init__(self, process_model: Union[StationProcessModel, ModelProxy]) -> None:
        ...
        self.STATES = [State(name='init_state'),
                       ...
                       State(name='add_liquid_1', on_enter=[
                             'request_adding_liquid_1']),

                       State(name='add_solid', on_enter=[
                           'request_solid_addition_process']),

                       State(name='sample_reaction', on_enter=[
                           'request_sample_reaction']),
                       State(name='start_analysis_process',
                             on_enter=['request_analysis_process']),
                       State(name='start_adding_liquid_2', on_enter=[
                           'request_start_adding_liquid_2']),

                       State(name='run_reaction', on_enter=[
                             'request_reaction_start']),

                       State(name='wait_for_result'),

                       State(name='stop_adding_liquid_2', on_enter=[
                           'request_stop_adding_liquid_2']),
                       State(name='stop_reaction', on_enter=[
                             'request_reaction_stop']),
        ...
        self.TRANSITIONS = [
            ...
            {'source': 'add_liquid_1', 'dest': 'add_solid',
                'conditions': 'are_req_station_ops_completed'},
            {'source': 'add_solid', 'dest': 'start_adding_liquid_2',
                'conditions': 'are_req_station_procs_completed'},

            {'source': 'start_adding_liquid_2', 'dest': 'sample_reaction',
                'conditions': 'are_req_station_ops_completed'},

            {'source': 'sample_reaction', 'dest': 'start_analysis_process',
                'conditions': 'are_req_station_ops_completed'},
            {'source': 'start_analysis_process', 'dest': 'run_reaction'},
            {'source': 'run_reaction', 'dest': 'wait_for_result',
                'conditions': 'are_req_station_ops_completed'},
            {'source': 'wait_for_result', 'dest': 'sample_reaction',
                'unless': 'is_reaction_complete', 'conditions': 'are_req_station_procs_completed'},
            {'source': 'wait_for_result', 'dest': 'stop_adding_liquid_2', 'conditions': [
                'is_reaction_complete', 'are_req_station_procs_completed']},

            {'source': 'stop_adding_liquid_2', 'dest': 'stop_reaction',
                'conditions': 'are_req_station_ops_completed'},
            {'source': 'stop_reaction', 'dest': 'final_state',
                'conditions': 'are_req_station_ops_completed'}
        ]
    ...
    def from_args(cls, lot: Lot,
        ...
        cls._set_model_common_fields(model,
                                     ...
                                     skip_station_ops,
                                     skip_ext_procs)
        model.data["target_product_concentration"] = float(
            target_product_concentration)
    ...
    def request_adding_liquid_1(self):
        ...
        current_op = self.generate_operation(
            f"add_liquid_1", target_sample=sample)
    ...
    def request_start_adding_liquid_2(self):
        ...
        sample = self.lot.batches[batch_index].samples[sample_index]
        current_op = self.generate_operation(
            f"dispense_liquid_2", target_sample=sample)
        self.request_station_op(current_op)
...
class APCFiltrationProcess(StationProcess):
    def __init__(self, process_model: Union[StationProcessModel, ModelProxy]) -> None:
        ...
        self.STATES = [State(name='init_state'),
                       ...
                       State(name='start_heat_stir', on_enter=[
                           'request_heating_stirring']),
                       State(name='open_close_drain_valve', on_enter=[
                           'request_open_close_drain_valve']),
                       State(name='increment_discharge_cycle', on_enter=[
                           'request_discharge_cycle_update']),
                       State(name='filter_product', on_enter=[
                             'request_filtration']),
                       State(name='add_wash_liquid', on_enter=[
                           'request_adding_wash_liquid']),
                       State(name='stop_heat_stir', on_enter=[
                           'request_stop_heating_stirring']),
                       State(name='dry_product', on_enter=[
                             'request_dry_product']),
                       State(name='final_state')]
        ...
        self.TRANSITIONS = [
            ...
            {'source': 'start_heat_stir', 'dest': 'open_close_drain_valve',
                'conditions': 'are_req_station_ops_completed'},

            {'source': 'open_close_drain_valve', 'dest': 'increment_discharge_cycle',
                'conditions': 'are_req_station_ops_completed'},
            ...
            {'source': 'add_wash_liquid', 'dest': 'open_close_drain_valve',
                'conditions': 'are_req_station_ops_completed'},

            {'source': 'filter_product', 'dest': 'stop_heat_stir',
                'conditions': ['are_req_station_ops_completed', "is_product_discharged", "is_vessel_clean"]},
            {'source': 'stop_heat_stir', 'dest': 'dry_product',
                'conditions': 'are_req_station_ops_completed'},
            {'source': 'dry_product', 'dest': 'final_state',
                'conditions': 'are_req_station_ops_completed'},
        ]
    ...
    def from_args(cls, lot: Lot,
        ...
        model.data["max_num_cycles"] = int(
            num_wash_cycles) + int(num_discharge_cycles)
        model.data["target_batch_index"] = int(target_batch_index)
        model.data["target_sample_index"] = int(target_sample_index)
        model.save()
        return cls(model)
    ...
    def request_heating_stirring(self):
        ...
        sample = self.lot.batches[batch_index].samples[sample_index]
        current_op = self.generate_operation(
            "heat_stir_discharge", target_sample=sample)
        self.request_station_op(current_op)
    ...
    def request_adding_wash_liquid(self):
        ...
        station_op = self.generate_operation(
            "wash_product", target_sample=sample)
        self.request_station_op(station_op)
    ...
    def request_dry_product(self):
        ...
        sample = self.lot.batches[batch_index].samples[sample_index]
        station_op = self.generate_operation(
            "dry_product", target_sample=sample)
        self.request_station_op(station_op)
...
class APCCleaningProcess(StationProcess):
    def __init__(self, process_model: Union[StationProcessModel, ModelProxy]) -> None:
        ...
        self.STATES = [State(name='init_state'),
                       ...
                       State(name='load_cleaning_funnel', on_enter=[
                           'request_load_cleaning_funnel']),
                       State(name='add_wash_liquid', on_enter=[
                           'request_adding_wash_liquid']),
                       ...
                       State(name='open_close_drain_valve', on_enter=[
                           'request_open_close_drain_valve']),
                       State(name='unload_cleaning_funnel', on_enter=[
                           'request_unload_cleaning_funnel']),
                       State(name='drain_filter', on_enter=[
                             'request_filter_drain']),
                       State(name='final_state')]
        ...
        self.TRANSITIONS = [
            ...
            {'source': 'load_cleaning_funnel', 'dest': 'add_wash_liquid',
                'conditions': 'are_req_robot_ops_completed'},
            {'source': 'add_wash_liquid', 'dest': 'run_reaction',
                'conditions': 'are_req_station_ops_completed'},
            {'source': 'run_reaction', 'dest': 'stop_reaction',
                'conditions': 'are_req_station_ops_completed'},
            {'source': 'stop_reaction', 'dest': 'sample_reaction',
                'conditions': 'are_req_station_ops_completed'},
            {'source': 'sample_reaction', 'dest': 'start_analysis_process',
                'conditions': 'are_req_station_ops_completed'},
            {'source': 'start_analysis_process', 'dest': 'open_close_drain_valve',
                'conditions': 'are_req_station_procs_completed'},
            {'source': 'open_close_drain_valve', 'dest': 'drain_filter',
                'conditions': 'are_req_station_ops_completed'},
            {'source': 'drain_filter', 'dest': 'add_wash_liquid',
                'unless': 'is_reactor_clean', 'conditions': 'are_req_station_ops_completed'},
            {'source': 'drain_filter', 'dest': 'unload_cleaning_funnel', 'conditions': [
                'are_req_station_ops_completed', 'is_reactor_clean']},
            {'source': 'unload_cleaning_funnel', 'dest': 'final_state',
                'conditions': 'are_req_robot_ops_completed'},
        ]
    ...
    def from_args(cls, lot: Lot,
        ...
        cls._set_model_common_fields(model,
                                     ...
                                     skip_station_ops,
                                     skip_ext_procs)
        model.data["target_purity_concentration"] = float(
            target_purity_concentration)
        model.data["target_batch_index"] = int(target_batch_index)
        model.data["target_sample_index"] = int(target_sample_index)
        model.save()
        return cls(model)
    ...
    def request_adding_wash_liquid(self):
        ...
        sample = self.lot.batches[batch_index].samples[sample_index]
        station_op = self.generate_operation(
            "add_wash_liquid", target_sample=sample)
        self.request_station_op(station_op)
    ...
    def request_reaction_start(self):
        ...
        sample = self.lot.batches[batch_index].samples[sample_index]
        current_op = self.generate_operation(
            "wash_heat_stir", target_sample=sample)
        self.request_station_op(current_op)
...
class APCMeasureYieldProcess(StationProcess):
    ...
    def __init__(self, process_model: Union[StationProcessModel, ModelProxy]) -> None:
        ...
        self.STATES = [
            ...
            State(name='prep_state'),
            State(name='navigate_to_weighing_station',
                  on_enter=['request_navigate_to_weighing']),
        ...
        self.TRANSITIONS = [
            ...
            {'source': 'navigate_to_weighing_station', 'dest': 'open_sash',
                'conditions': 'are_req_robot_ops_completed'},
            {'source': 'open_sash', 'dest': 'weight_yield',
                'conditions': 'are_req_station_ops_completed'},
            {'source': 'weight_yield', 'dest': 'close_sash',
                'conditions': 'are_req_station_procs_completed'},
            {'source': 'close_sash', 'dest': 'final_state',
                'conditions': 'are_req_station_ops_completed'}
        ]
