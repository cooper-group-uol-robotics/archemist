...
class YumiShakerPlateSm(StationProcessFSM):
    def __init__(self, station: Station, params_dict: Dict):
        ...
        transitions = [
            ...
            },
            {
                ...
                "dest": "load_shaker_plate",
                "conditions": "all_batches_assigned",
            },
            {
                "trigger": self._trigger_function,
                "source": "load_shaker_plate",
                "dest": "shake",
            ...
            },
            {
                "trigger": self._trigger_function,
                "source": "shake",
                "dest": "unload_shaker_plate",
                "conditions": "is_station_job_ready",
            },
            {
                "trigger": self._trigger_function,
                "source": "unload_shaker_plate",
                "dest": "unscrew_caps",
                "conditions": "is_station_job_ready",
            },
            {
            ...
            {
                "trigger": self._trigger_function,
                "source": "pick_pxrd_rack",
                "dest": "pick_eightw_rack",
                "conditions": "is_station_job_ready",
            },
            {
                "trigger": self._trigger_function,
                "source": "pick_eightw_rack",
    ...
    def request_load_shaker_plate(self):
        robot_job = YuMiRobotTask.from_args(
            name="invertAndLoadShakerPlate", location=self._station.location
        )
    ...
    def request_unload_shaker_plate(self):
        robot_job = YuMiRobotTask.from_args(
            name="invertAndLoadWellPlate", location=self._station.location
        )
        current_batch_id = self._station.assigned_batches[
            self._status["batch_index"]
        ].id
    ...
    def request_unscrew_caps(self):
        robot_job = YuMiRobotTask.from_args(
            name="unscrewCaps", location=self._station.location
        )
        current_batch_id = self._station.assigned_batches[
            self._status["batch_index"]
        ].id
